...
class GSeMTinyVit(nn.Module):
    def __init__(
        ...
        channels = 64
        self.conv1 = nn.Conv2d(in_chans, channels, kernel_size=3, stride=1, padding=1, bias=False)
        self.bn1 = nn.BatchNorm2d(channels)
        self.gsem_module = GradSemiMaskModule(channels, alpha, beta)
...
def GSeMTinyVit_11M(num_classes=10, alpha=6, beta=4):
    return GSeMTinyVit(
...
def GSeMTinyVit_21M(num_classes=10, alpha=6, beta=4):
    return GSeMTinyVit(
        num_classes=num_classes,
        alpha=alpha,
        beta=beta,
